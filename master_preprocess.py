...
usage="Usage: %prog {-i <single> | -1 <mate1> -2 <mate2>} -g <genome.fa> [options]"


-h 會出來   Usage: bs_seeker2-align.py {-i <single> | -1 <mate1> -2 <mate2>} -g <genome.fa> [options]
...
##首先我要把name灌入，然後只有metaplot是獨立需要bw的
##存下bz file 的名字 >>>阿拉伯屆只有10min30s
def cgmap2bwbz(name,cgfile,cutoff):
    """
    cut the depth of cgmap, and save
    """
    if cgfile[-3:]==".gz":
    	cgmap=pd.read_csv(cgfile,sep='\t',header=None,dtype={0:str},compression='gzip')  #### time is long
    elif cgfile[-6:]==".CGmap":
    ...
    if len(cgmap_cutoff)>0:
    	...
    	print "no file"
    if cgfile[-3:]==".gz":
        cgmap_cutoff.to_csv("cutoff%d_%s"%(cutoff,cgfile),header=False,sep="\t",index=False,compression='gzip') ##用成gzip比較久
        print "save file cutoff%d_%s"%(cutoff,cgfile)
        bzfile=cgfile[:-6]+"bz"
    ...
    ## open bw file to write>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> here should save bw file
    ...
    for chromosome in cgmapc.iloc[:,0].unique():
    	...
    	bw_CG.addEntries(chromosome,[x-1 for x in CG.ix[:,2].tolist()],values=[float(f) for f in CG.ix[:,5].tolist()], span=1)##span is
...
def bw_all(bw_CG,bw_CHG,bw_CHH):
...
###gff3to gtf>>>gffread my.gff3 -T -o my.gtf
####gtf processing
...
def AllinfoAnnotation(genePred):
    ...
    #subprocess.check_output("cat OTU5extended_genePred|awk \'OFS=\"\t\"{print $12,$1,$2,$3,$4,$5,$5-$4,$8,$9,$10}\'>all_info_annot0712",shell=True)
    #cat genePred|awk 'OFS="\t"{print $12,$1,$2,$3,$4,$5,$5-$4,$8,$9,$10}'#>all_info_annot"
...
##改版的>>>43s>>>注意>>1. list()是將tuple轉成list 2. list apeend不用只是變數 3.column最後加比較好
...
###兩種方式1. subprocess直接寫入 2. 用list去append(cgbz的還沒試)
最初ok的舊版
def func_Meth_siteC_siteCT(Meth_file,chr,left,right,cutoff):   ##calculate the number of C and the sum of all methylation level in the exon
    subprocess.call("cgmaptools fetch cgbz -b %s -C %s -L %d -R %d >findCGmapfile"%(Meth_file,chr,left,right),shell=True)
    cgmap_select=pd.read_csv('findCGmapfile', sep='\t',names=["chr","1Letter","site","3letter","2letter","Meth_level","numC","numC_T"],dtype={"chr":str})
...
file=CCMC0031_R1_rmdup.fastq.gz_bspe.bam.CGbz
chr="1"


### try1>>> subprocess into shell right away>>>>bz最好去過cutoff的
command_line ="cgmaptools fetch cgbz -b %s -C %s -L %d -R %d"%(Meth_file,chr,left,right)
args = shlex.split(command_line)       
a=subprocess.check_output(args, stdin=None, stderr=None, shell=False, universal_newlines=False)#print(a)#也是看起來是dataframe
#b=a.replace("\t",",")#好像不用
...
for i in range(len(c)):
    c[i]=c[i].split()
cgmap_select=pd.DataFrame(c,columns=["chr","1Letter","site","3letter","2letter","Meth_level","numC","numC_T"])#numpy就很好用了阿,pandas更好用sep='\t',names=["chr","1Letter","site","3letter","2letter","Meth_level","numC","numC_T"],dtype={"chr":str}





def RunMethExon(Meth_file,extron_bed):
    ID_sumC_CT=pd.DataFrame(columns=['gene_ID','siteC_CG',"siteCT_CG","siteC_CHG","siteCT_CHG","siteC_CHH","siteCT_CHH"]) ## create all value for all exons in the bed. file 
    for i in range(len(extron_bed)):
        Meth_siteC_siteCT=func_Meth_siteC_siteCT("%s"%(Meth_file),extron_bed.iloc[i,0],extron_bed.iloc[i,1],extron_bed.iloc[i,2],4)
        row_value=pd.Series([extron_bed.iloc[i,3],Meth_siteC_siteCT["CG"][0],Meth_siteC_siteCT["CG"][1],Meth_siteC_siteCT["CHG"][0],Meth_siteC_siteCT["CHG"][1],Meth_siteC_siteCT["CHH"][0],Meth_siteC_siteCT["CHH"][1]],
             index=['gene_ID','siteC_CG',"siteCT_CG","siteC_CHG","siteCT_CHG","siteC_CHH","siteCT_CHH"])
        ID_sumC_CT=ID_sumC_CT.append(row_value,ignore_index=True)
    AverageMethExon=pd.DataFrame(columns=["gene_ID","Meth_value_mean_CG","Meth_value_mean_CHG","Meth_value_mean_CHH"]) ## calculate average methylation level of each gene
    for j in ID_sumC_CT["gene_ID"].unique():
        ...
        row_exon_mean=pd.Series([j,Meth_value_mean_CG,Meth_value_mean_CHG,Meth_value_mean_CHH],
            index=["gene_ID","Meth_value_mean_CG","Meth_value_mean_CHG","Meth_value_mean_CHH"])
        AverageMethExon=AverageMethExon.append(row_exon_mean,ignore_index=True)
...
##user只提供一個bed呢??
#還要把expression放進來
#def BedAddExp(name,exp,gb_bed, pmt_bed, exon_bed, intron_bed):
...
### meth & exp >>>用意是同一個gene annotation file 產生bed只要一次就好
def WholeProcess(name, cgmap, cutoff, exp, gb_bed, pmt_bed, exon_bed, intron_bed):
    ...
    print localtime
    #t12 = time.time()
    #return GeneMeth, PromoterMeth, Exon_meth, Intron_meth ##cgmap_cutoff,bwall

#WholeProcess(name, cgmap, cutoff, exp, gb_bed, pmt_bed, exon_bed, intron_bed)
...
"""
    ##cgmap process
    cgmap_cutoff,bwall=cgmap2bw(args.samplename,args.cgmapfilename,args.cutoff)
    ##gtf process
    genePred=gtfToGenePred(args.gtffile)
    ...
    ### average meth in target region
    GeneMeth=RunMethGeneBw(bwall,gb_bed)
    GeneMeth.to_csv("GeneMean.txt",sep="\t",index=False,header=False)
    print "save genemean"
    PromoterMeth=RunMethGeneBw(bwall,pmt_bed)
    PromoterMeth.to_csv("PromoterMean.txt",sep="\t",index=False,header=False)
    print "save PromoterMean"
    exon_bed=pd.read_csv("exon_bed",sep="\t",header=None,dtype={0:str})
    intron_bed=pd.read_csv("intron_bed",sep="\t",header=None,dtype={0:str})
    Exon_meth=RunMethExon(cgmap_cutoff,exon_bed)
    Exon_meth.to_csv("ExonMean.txt",sep="\t",index=False,header=False)
    print "save exon mean"
    Intron_meth=RunMethExon(cgmap_cutoff,intron_bed)
    Intron_meth.to_csv("IntronMean.txt",sep="\t",index=False,header=False)
    print "save intronmean"
